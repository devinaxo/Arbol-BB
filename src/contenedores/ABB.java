...
public abstract class ABB extends ArbolBinario{
	...
	public NodoArbolBinario devuelve(Object nodoInfo){
		...
		if (!estaVacio()){
			nodo=devuelveNodo(nodoInfo);			
			if (nodo==null){
				System.out.println("Error devuelve(). No se encuentra");
			}			
		}else{
		...
		if (!estaVacio()){
			if (esta(nodoInfo)){
				...
				if (temp.getLeftChild()==null && temp.getRightChild()==null){
					...
					if (anterior==null){ //mi nodo a eliminar es una hoja que también es raíz
						limpiar();
					}else{
				...
				System.out.println("Error sacar(). No esta el elemento.");
			}
		}else{
